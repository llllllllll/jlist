...
from jlist.tests import seq_tests
...
class CommonTest(seq_tests.CommonTest):
    ...
    def test_getitem_error(self):
        a = self.type2test([])
        msg = "jlist indices must be integers or slices"
    ...
    def test_repr(self):
        ...
        self.assertEqual(str(a0), "jlist([])")
        self.assertEqual(repr(a0), "jlist([])")
        self.assertEqual(str(a2), "jlist([0, 1, 2])")
        self.assertEqual(repr(a2), "jlist([0, 1, 2])")
    ...
    def test_reversed(self):
        a = self.type2test(range(20))
        r = reversed(a)
        self.assertEqual(self.type2test(r), self.type2test(range(19, -1, -1)))
    ...
    def test_setslice(self):
        ...
        for i in range(-3, 4):
            ...
            a2 = a[:]
            a2[:i] = a[:i]
            self.assertEqual(a2, self.type2test(a))
            a[i:] = l[i:]
            self.assertEqual(a, self.type2test(l))
            a2 = a[:]
            a2[i:] = a[i:]
    ...
    def test_extend(self):
        ...
        a.extend(CustomIter())
        self.assertEqual(a, self.type2test([1,2,3,4]))
    ...
    def test_copy(self):
        ...
        v = u.copy()
        v.append('i')
        self.assertEqual(u, self.type2test(['a', 'b']))
